...
public class Library_XmlToJson {

    private static final String TAG = "Library_XmlToJson";
    ...
    // default values when a Library_Tag is empty
    ...
    /**
     * Builder class to create a Library_XmlToJson object
     */
    public static class Builder {
        ...
        /**
         * Force a XML Library_Tag to be interpreted as a list
         *
         ...
         *
         * @param contentPath     Path for the Library_Tag that holds the content, using format like "/parentTag/childTag"
         ...
         * @param path Path for the Library_Tag content or Attribute, using format like "/parentTag/childTag"
         * @return the Builder
         */
        ...
        /**
         * Skips a Library_Tag (will not be present in the JSON)
         *
         * @param path Path for the Library_Tag, using format like "/parentTag/childTag"
         * @return the Builder
         */
        ...
        /**
         * Creates the Library_XmlToJson object
         *
         * @return a Library_XmlToJson instance
         */
        public Library_XmlToJson build() {
            return new Library_XmlToJson(this);
        }
    }
    ...
    private Library_XmlToJson(Builder builder) {
    ...
    JSONObject convertToJSONObject() {
        try {
            Library_Tag parentLibraryTag = new Library_Tag("", "xml");
            ...
            readTags(parentLibraryTag, xpp);

            unsetInput();

            return convertTagToJson(parentLibraryTag, false);
    ...
    private void readTags(Library_Tag parent, XmlPullParser xpp) {
        try {
            int eventType;
            do {
                eventType = xpp.next();
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    Library_Tag child = new Library_Tag(path, tagName);
                    if (!skipTag) {
                    ...
                    for (int i = 0; i < attrCount; ++i) {
                        ...
                        Library_Tag attribute = new Library_Tag(attrPath, attrName);
    ...
    private JSONObject convertTagToJson(Library_Tag libraryTag, boolean isListElement) {
        JSONObject json = new JSONObject();

        // Content is injected as a key/value
        if (libraryTag.getContent() != null) {
            String path = libraryTag.getPath();
            String name = getContentNameReplacement(path, DEFAULT_CONTENT_NAME);
            putContent(path, json, name, libraryTag.getContent());
        }

        try {

            HashMap<String, ArrayList<Library_Tag>> groups = libraryTag.getGroupedElements(); // groups by libraryTag names so that we can detect lists or single elements
            for (ArrayList<Library_Tag> group : groups.values()) {

                if (group.size() == 1) {    // element, or list of 1
                    Library_Tag child = group.get(0);
    ...
    private boolean isForcedList(Library_Tag libraryTag) {
        String path = libraryTag.getPath();
